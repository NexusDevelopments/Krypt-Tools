...
function getTicketPanelPayload(config) {
  const embed = new EmbedBuilder()
    .setColor(LIGHT_BLUE)
    .setTitle(config.panelTitle || 'Krypt Tickets')
    .setDescription(
      config.panelDescription ||
      'Need help? Click **Open Ticket** and Krypt Support will assist you.'
    )
    .setTimestamp();
...
function buildTicketStatusEmbed(ticketMeta) {
  return new EmbedBuilder()
    ...
    .setDescription('Krypt Support will be with you shortly. Use the controls below to manage this ticket.')
    .addFields(
  ...
  if (content.startsWith(`${PREFIX}ticket`)) {
    ...
    if (subcommand === 'help') {
      const ticketHelp = new EmbedBuilder()
        .setColor(LIGHT_BLUE)
        .setTitle('Krypt Ticket Commands')
    ...
    if (subcommand === 'setup') {
      ...
      setTicketConfig(message.guild.id, {
        ...
        panelTitle: 'Krypt Tickets',
        panelDescription: 'Need help? Click **Open Ticket** and Krypt Support will assist you.'
      });
...
app.post('/api/tickets/config', (req, res) => {
  ...
  setTicketConfig(guildId, {
    ...
    supportRoleId: supportRoleId || null,
    logChannelId: logChannelId || null,
    panelTitle: String(req.body.panelTitle || 'Krypt Tickets').slice(0, 120),
    panelDescription: String(req.body.panelDescription || 'Need help? Click **Open Ticket** and Krypt Support will assist you.').slice(0, 1500)
  });
